expModelCmp = function(){
  ...
  bestFitNums = apply(waic_[allPass,], 1, which.min)
  ...
  plotData = data.frame(
    modelName = c("RL", "BL", "ON"),
    bestFitNum = c(sum(bestFitDf$bestFitNum[bestFitDf$modelName %in% c('QL1', 'QL2', 'RL1', 'RL2')]), bestFitDf$bestFitNum[bestFitDf$modelName == 'BL'], bestFitDf$bestFitNum[bestFitDf$modelName == 'optim_noise'])
  )
  pattern.type<-c('vdashes', 'blank', 'blank')
  ...
  patternpie(group = plotData$modelName , pct = plotData$prop,
             ...
             pattern.color = c("#33a02c", "black", "black"),
             pattern.line.size=c(30, 10, 10), frame.color='black',frame.size=1.5, pixel=12, density=c(10, 10, 10),
             background.color = c("#a6cee3", "#fb9a99","#e31a1c")) 
