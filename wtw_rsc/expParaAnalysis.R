...
plotData$para = recode(plotData$para,
                       alphaR = "alpha[r]",
                       alphaU = "alpha[u]",
                       prior = 'eta')
plotData %>% mutate(para = factor(para, levels = unique(plotData$para))) %>%
  ggplot(aes(value)) + geom_histogram(bins = 6) +
...
# plot correlations 
##### plot correlations 
expPara$auc = sumStats$muWTW
expPara$cip = sumStats$stdWTW
plotData = expPara %>% select(c("alphaR", "alphaU", "tau", "gamma", "prior", "auc", "cip", "condition")) %>%
  filter(passCheck)

for(i in 1 : 2){
  condition = conditions[i]
  corMx = cor(plotData[plotData$condition == condition, 1 : ncol(plotData) - 1])
...
expPara$auc = sumStats$muWTW
expPara$cip = sumStats$stdWTW
plotData = expPara %>% select(c("alphaR", "alphaU", "tau", "gamma", "prior", "condition")) %>%
  filter(passCheck)

corMx = cor(plotData[, 1 : ncol(plotData) - 1])
corrplot(corMx, method="number", type="upper")
